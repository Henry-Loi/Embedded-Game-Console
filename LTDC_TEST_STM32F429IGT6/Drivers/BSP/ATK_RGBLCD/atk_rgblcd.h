...
#define ATK_RGBLCD_BL_GPIO_CLK_ENABLE() do{ __HAL_RCC_GPIOB_CLK_ENABLE(); }while(0)
#define ATK_RGBLCD_M0_GPIO_PORT         GPIOG
#define ATK_RGBLCD_M0_GPIO_PIN          GPIO_PIN_6
#define ATK_RGBLCD_M0_GPIO_CLK_ENABLE() do{ __HAL_RCC_GPIOG_CLK_ENABLE(); }while(0)
#define ATK_RGBLCD_M1_GPIO_PORT         GPIOI
#define ATK_RGBLCD_M1_GPIO_PIN          GPIO_PIN_2
#define ATK_RGBLCD_M1_GPIO_CLK_ENABLE() do{ __HAL_RCC_GPIOI_CLK_ENABLE(); }while(0)
#define ATK_RGBLCD_M2_GPIO_PORT         GPIOI
#define ATK_RGBLCD_M2_GPIO_PIN          GPIO_PIN_7
#define ATK_RGBLCD_M2_GPIO_CLK_ENABLE() do{ __HAL_RCC_GPIOI_CLK_ENABLE(); }while(0)

/* IO操作 */
#define ATK_RGBLCD_BL(x)                do{ x ?                                                                                 \
                                            HAL_GPIO_WritePin(ATK_RGBLCD_BL_GPIO_PORT, ATK_RGBLCD_BL_GPIO_PIN, GPIO_PIN_SET) :  \
...
void atk_rgblcd_draw_line(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color);                                                          /* RGB LCD模块LCD画线段 */
void atk_rgblcd_draw_rect(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color);                                                          /* RGB LCD模块LCD画矩形框 */
void atk_rgblcd_draw_circle(uint16_t x, uint16_t y, uint16_t r, uint16_t color);                                                                        /* RGB LCD模块LCD画圆形框 */
void atk_rgblcd_show_char(uint16_t x, uint16_t y, char ch, atk_rgblcd_lcd_font_t font, uint16_t color);                                                 /* RGB LCD模块LCD显示1个字符 */
void atk_rgblcd_show_string(uint16_t x, uint16_t y, uint16_t width, uint16_t height, char *str, atk_rgblcd_lcd_font_t font, uint16_t color);            /* RGB LCD模块LCD显示字符串 */
void atk_rgblcd_show_xnum(uint16_t x, uint16_t y, uint32_t num, uint8_t len, atk_rgblcd_num_mode_t mode, atk_rgblcd_lcd_font_t font, uint16_t color);   /* RGB LCD模块LCD显示数字，可控制显示高位0 */
void atk_rgblcd_show_num(uint16_t x, uint16_t y, uint32_t num, uint8_t len, atk_rgblcd_lcd_font_t font, uint16_t color);                                /* RGB LCD模块LCD显示数字，不显示高位0 */
