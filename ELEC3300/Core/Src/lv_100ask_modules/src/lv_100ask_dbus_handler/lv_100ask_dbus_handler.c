...
// export $(dbus-launch)
// show dbus servers list: dbus-send --print-reply --dest=org.freedesktop.DBus  /org/freedesktop/DBus
// org.freedesktop.DBus.ListNames test command: dbus-send --session --type=method_call --print-reply
// --dest=net.ask100.lvgl.sum /net/ask100/lvgl/sum net.ask100.lvgl.sum.Sum int32:100 int32:999
...
static DBusHandlerResult tutorial_messages(DBusConnection* connection, DBusMessage* message, void* user_data) {
	...
	} else if (0 == strcmp(g_Str_object_path, interface_name) && 0 == strcmp("states", member_name)) {
...
static void* dbus_dispatch_thread_func(void* pvoid) {
	...
	#if 1
void dbus_method_call(const char* destination, const char* path, const char* iface, const char* method, const int state,
					  const int pid) {
...
void lv_100ask_dbus_handler_init(const char* path, const char* iface) {
	...
	// 尝试注册一个新的对象为当前应用程序
	dbus_connection_try_register_object_path(onnection, g_Str_object_iface, &vtable, NULL, &error);
	check_and_abort(&error);
