...
static void print_matrix(void) {
	for (int y = 0; y < MAX_Y; y++) {
		for (int x = 0; x < MAX_X; x++) {
			...
			tft_fill_rect(x * BLOCK_LEN + 1, y * BLOCK_LEN + 1 + TOP_PADDING, (x + 1) * BLOCK_LEN - 1,
						  (y + 1) * BLOCK_LEN - 1 + TOP_PADDING, curr_color);
			tft_fill_rect(x * BLOCK_LEN, y * BLOCK_LEN + TOP_PADDING, (x + 1) * BLOCK_LEN,
						  y * BLOCK_LEN + 1 + TOP_PADDING, BLACK);
			tft_fill_rect(x * BLOCK_LEN, y * BLOCK_LEN + TOP_PADDING, x * BLOCK_LEN,
						  (y + 1) * BLOCK_LEN + 1 + TOP_PADDING, BLACK);
			tft_fill_rect((x + 1) * BLOCK_LEN, y * BLOCK_LEN + TOP_PADDING, (x + 1) * BLOCK_LEN + 1,
						  (y + 1) * BLOCK_LEN + 1 + TOP_PADDING, BLACK);
			tft_fill_rect(x * BLOCK_LEN, (y + 1) * BLOCK_LEN + TOP_PADDING, (x + 1) * BLOCK_LEN + 1,
						  (y + 1) * BLOCK_LEN + 1 + TOP_PADDING, BLACK);
		}
	}
	// else {
...
static void draw_windows(void) {
	...
	int game_win_start_x = center - MAX_X * 2 - 1;
	int game_win_start_y = title_win_height + 2;
	int game_win_width = MAX_X * 2 + 2;
	int game_win_height = MAX_Y + 2;

	int score_win_start_x = game_win_start_x + game_win_width + 3;
	int score_win_start_y = game_win_start_y;
	int score_win_width = MAX_X - score_win_start_x;
...
static void main_loop(void) {
	...
	while (1) {
		...
		/* 	switch (getch()) {
				...
				case 'q':
					return;
					// case KEY_RESIZE: destroy_windows(); draw_windows();
			 }*/
		// refresh_screen();
	}
}
...
void tetris_thread(void* par) {
	...
	while (1) {
		...
		if (ctrller.button[L_BTN_UP]) {
			rotate();
		} else if (ctrller.button[L_BTN_LEFT]) {
			move_left();
		} else if (ctrller.button[L_BTN_RIGHT]) {
			move_right();
		} else if (ctrller.button[L_BTN_DOWN]) {
			move_bottom();
		}
		tft_update();
		// draw_windows();
