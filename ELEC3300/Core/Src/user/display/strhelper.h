 ...
 */
static char* itos(char* data, int32_t i) {
	if (i < 0) {
		i = -i;
		*data++ = '-';
	}
 ...
 * @return     Returns the new end pointer of the string
 */
static char* ltos(char* data, int64_t i) {
	if (i < 0) {
		i = -i;
		*data++ = '-';
	}
 ...
 * @return     Returns the new end pointer of the string
 */
static char* utos(char* data, uint32_t i) {
	char buf[14] = {0};
	char* ptr = &buf[1];
	do {
		*ptr++ = '0' + (i % 10);
 ...
 * @return     Returns the new end pointer of the string
 */
static char* itos2(char* data, int32_t i) {
	if (i < 0) {
		i = -i;
		*data++ = '-';
	}
...
/**
 * @brief      Convert specific number of (most significant first) digits of an
 * unsigned integer to a string
 *
 * If the number has less digits than specified then it is left--padded with
 * zeros
 *
 * @param      data    The string to write to
 ...
 * @return     Returns the new end pointer of the string
 */
static char* utos0(char* data, uint32_t i, uint32_t digits) {
	char buf[20] = {0};
	char* ptr = &buf[1];
	digits--;
	uint32_t n = 0;
...
static char* utos02(char* data, uint64_t i, uint32_t digits) {
	char buf[40] = {0};
	char* ptr = &buf[1];
	digits--;
	uint32_t n = 0;
...
/**
 * @brief      Converts a float to a string with a certain number of decimal
 * places
 *
 ...
 * @return     Returns the new end pointer of the string
 */
static char* ftos(char* data, float a, uint32_t dp) {
	int64_t i = a;
	if (a < 0) {
		*data++ = '-';
		i = -i;
	...
	*data++ = '.';
	...
	if (__dp != 0)
		data = utos02(data, (uint64_t)(d), __dp);
	return data;
}
 ...
 * @return     Returns the new end pointer of the string
 */
static char* ftos2(char* data, float a, uint32_t dp) {
	int32_t i = a;
	if (a < 0) {
		*data++ = '-';
		i = -i;
...
static int print(char** out, const char* format, va_list args) {
	...
	for (; *format != 0; ++format) {
		if (*format == '%') {
			...
			if (*format == 'f') {
				pc += printf_(out, va_arg(args, double), width, dp ? dp : 3, pad);
				continue;
			}
		} else {
		out:
