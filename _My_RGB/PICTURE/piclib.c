...
void piclib_draw_hline(u16 x0,u16 y0,u16 len,u16 color)
{
	...
	Paint_DrawLine(x0, y0, x0+len-1, y0, color, LINE_STYLE_SOLID, DOT_PIXEL_2X2);
...
{  
	Paint_ClearWindows(x, y, x+width, y+height, *color);
...
{
	pic_phy.read_point=BSP_LCD_ReadPixel;  		//读点函数实现,仅BMP需要
	pic_phy.draw_point=Paint_DrawPoint;	//画点函数实现
	pic_phy.fill=Paint_ClearWindows;					//填充函数实现,仅GIF需要
