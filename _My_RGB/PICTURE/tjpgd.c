...
{ 
   	...
   	for(i=0;i<height;i++)//y坐标
	{
		...
		for(j=0;j<width;j++)//x坐标
		{
			...
			*pic_temp=*pencolor;
			pic_temp++;
			pic_phy.draw_point(realx+picinfo.S_XOFF,realy+picinfo.S_YOFF,*pencolor,DOT_PIXEL_1X1,DOT_FILL_AROUND);//显示图片						
			pencolor++;
		}
	}
    return 0;    //返回0,使得解码工作继续执行 
				...
				{ 
					...
					{	
						if(((jpeg_dev->width>>scale)!=picinfo.S_Width)&&((jpeg_dev->height>>scale)!=picinfo.S_Height&&scale))
							...
							outfun=jpeg_out_func_point;	//在显示尺寸以内,可以采用填充的方式显示 jpeg_out_func_fill//填充方式显示的图片会很模糊
						break; 							
					} 
				} 
