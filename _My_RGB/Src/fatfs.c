...
u8 exfuns_init(void)
{
	...
	if(fatbuf)return 0;  //申请有一个失败,即失败.
	else return 1;	
}
...
{		   
	return f_mount(&SDFatFS,(const TCHAR*)path,mt); 
}
//打开路径下的文件
//path:路径+文件名
//mode:打开模式
...
{
	u8 res;	 
	res=f_open(&SDFile,(const TCHAR*)path,mode);//打开文件夹
	return res;
} 
//关闭文件
//返回值:执行结果
...
u8 mf_read(u16 len)
{
	...
	for(i=0;i<len/512;i++)
	{
		res=f_read(&SDFile,fatbuf,512,&br);
		if(res)
		{
			printf("Read Error:%d\r\n",res);
			break;
	...
	if(len%512)
	{
		res=f_read(&SDFile,fatbuf,len%512,&br);
		if(res)	//读数据出错了
		{
			printf("\r\nRead Error:%d\r\n",res);   
		}else
...
u8 mf_readdir(void)
{
	...
	printf("dir.obj.objsize:%ld\r\n",dir.obj.objsize);
//	printf("dir.obj.c_ofs:%d\r\n",(uint32_t)dir.obj.c_ofs);
	printf("dir.clust:%ld\r\n",dir.clust);
	printf("dir.sect:%ld\r\n",dir.sect);	  
...
{
 	TCHAR* rbuf;
	rbuf=f_gets((TCHAR*)fatbuf,size,&SDFile);
